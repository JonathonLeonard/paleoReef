...
# Project at 120 Ma
# Read environmental dataset
# files <- list.files(path=paste0(envRasterPath, '/', co2, 'ppm/', time,'Ma_envRaster'), pattern='.tif', full.names=TRUE )
files <- unlist(strsplit(envRasters, ","))
print(files)
nb_env <- length(files)
print(nb_env)
...
reefBiomodProj <- BIOMOD_Projection(bm.mod = reefBiomodModel,
                                  proj.name = paste0(modelName, '_', time,'Ma'),
                                  new.env = enviVar,
                                  models.chosen = 'all',
                                  metric.binary = 'all',
                                  metric.filter = 'all',
